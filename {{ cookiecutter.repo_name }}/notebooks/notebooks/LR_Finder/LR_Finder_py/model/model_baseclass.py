...
class ModelBaseClass_Images(abc.ABC):
    ...
    def build_model(self, augmentation, rescaling, model, weights, num_classes, input_shape):
        #set params
        if num_classes == 2:
            last_act_fn = "sigmoid"
            num_classes = 1
        ...
        #Transfer learning (for model pool see -> https://keras.io/api/applications/)
            ...
            if weights == None: #no weights, so train ALL params, and add top layer
            ...
            else: #pretrained params, so train just top layer
